...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        // Fallback to local storage credential checks to avoid blocking mock simulation testing
        console.warn("Supabase Auth error. Attempting local storage validation fallback.", authError.message);
      ...
      if (data?.user) {
        // 2. Refresh Zustand state from active Supabase database
        await loadFromLocalStorage();

        const activeStudent = useStudentStore.getState().student;
        if (activeStudent && activeStudent.onboarding_complete) {
          router.push("/dashboard");
        } else {
  ...
  const handleOAuthSignIn = async (provider: "google" | "github") => {
    ...
    try {
      ...
      if (authError || !data?.url) {
        ...
        console.warn(`Supabase OAuth (${provider}) error: ${errorMsg}. Executing simulated login fallback.`);
        await loadFromLocalStorage();
        let activeStudent = useStudentStore.getState().student;
        ...
        if (!activeStudent) {
          ...
          activeStudent = useStudentStore.getState().student;
        }
      ...
      console.warn(`OAuth login exception: ${err?.message}. Executing simulated login fallback.`);
      await loadFromLocalStorage();
      let activeStudent = useStudentStore.getState().student;
    ...
    <main className="min-h-screen bg-bg-base flex flex-col justify-center items-center px-6 py-12 select-none antialiased text-text-primary relative overflow-hidden">
      ...
      <div className="w-full max-w-[420px] bg-bg-elevated border border-border rounded-lg p-8 shadow-warm relative overflow-hidden z-10">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          >
            <span>{loading ? "AUTHENTICATING..." : "SIGN IN TO PREP"}</span>
